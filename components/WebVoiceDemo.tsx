...
export default function WebVoiceDemo() {
      ...
      if (update?.transcript) {
        if (Array.isArray(update.transcript)) {
          const text = update.transcript
            .map((item: any) =>
              `${item.role === "agent" ? "AI" : "Utente"}: ${item.content}`
            )
            .join("\n");
          setTranscript(text);
        } else {
  ...
  return (
    <section id="demo-audio" className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
      <div className="rounded-[2rem] border border-slate-200/70 bg-white/90 p-8 shadow-[0_24px_70px_rgba(15,23,42,0.08)]">
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-4 text-sm text-slate-500">
              Inserisci la tua email e ricevi subito il riepilogo della chiamata.
            </p>
